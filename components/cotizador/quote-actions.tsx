...
export function QuoteActions({ quoteId, token, status }: { quoteId: number, token?: string, status?: string }) {
  ...
  const handleConfirmAddress = async (addressData: any) => {
    setShowAddressModal(false)
    setIsApproving(true)
    try {
      await adminAcceptQuote(quoteId, addressData)
      window.location.reload()
    } catch (e) {
      console.error(e)
  ...
  return (
    <>
      ...
      <AddressModal isOpen={showAddressModal} onClose={() => setShowAddressModal(false)} onConfirm={handleConfirmAddress} />
    </>
  )
}
