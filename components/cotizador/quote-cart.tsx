    ...
    <div className="absolute right-0 top-8 z-50 bg-slate-900 border border-brand-cyan/50 rounded shadow-[0_0_15px_rgba(0,163,255,0.2)] p-3 w-64 text-left">
      ...
      {mode === 'manual' ? (
         <div className="flex items-center gap-1 mb-2">
            ...
            <input autoFocus type="number" value={val} onChange={e => setVal(e.target.value)} onKeyDown={e => { if(e.key === 'Enter') { if(mode === 'manual') onSave(Number(val)); else onSave(baseForMargin * (1 + Number(val)/100)); } }}  className="w-full bg-slate-950 border border-slate-700 rounded px-2 py-1 text-xs text-white outline-none focus:border-brand-cyan" />
         </div>
      ) : (
         <div className="flex items-center gap-1 mb-2">
            <input autoFocus type="number" placeholder="Ej. 30" value={val} onChange={e => setVal(e.target.value)} onKeyDown={e => { if(e.key === 'Enter') { if(mode === 'manual') onSave(Number(val)); else onSave(baseForMargin * (1 + Number(val)/100)); } }}  className="w-full bg-slate-950 border border-slate-700 rounded px-2 py-1 text-xs text-white outline-none focus:border-brand-cyan" />
            <span className="text-brand-cyan">%</span>
         </div>
      )}
