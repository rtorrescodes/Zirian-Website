...
export function RegisterForm({ locale }: { locale: string }) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signInRes?.error) {
        setError(isEn ? 'Registered, but failed to log in' : 'Registrado, pero falló el inicio de sesión');
        setLoading(false);
      } else {
      ...
      <form onSubmit={handleRegister} className="space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={isEn ? 'Password (min. 6 chars)' : 'Contraseña (mín. 6 caracteres)'}
            className="w-full bg-slate-900 border border-slate-700 rounded-xl py-3 pl-10 pr-4 text-white placeholder-slate-500 focus:outline-none focus:border-brand-cyan focus:ring-1 focus:ring-brand-cyan transition-colors"
            required
            minLength={6}
          />
